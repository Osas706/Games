...
function moveShooter(e) {
  ...
  switch (e.key) {
    case 'ArrowLeft':
      if (currenShooterIndex % width !== 0)
        currenShooterIndex -= 1
      break

    case 'ArrowRight':
      if (currenShooterIndex % width < width - 1)
        currenShooterIndex += 1
      break
...
function shoot(e) {
  let laserId
  let currrentLaserIndex = currenShooterIndex;
  function moveLaser() {
    ...
    if(
      squares[currrentLaserIndex].classList.contains('invader')
      ) {
      squares[currrentLaserIndex].classList.remove('laser');
